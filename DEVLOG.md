...
- Generated depth-3 recurrences, leaf summaries, and a leaf comparison for
  `p7-form-2` and `p8-form-1`.
- Added `BOUNDARY_DEPTH3_NOTES.md`: P7 resolves into forced orientation leaves
  at depth 3, while P8 decomposes into two forced-first blocks plus a balanced
  `13/13` unseen core.
...
- Continued P11 on `R013`.
- Added `matrix-vector-frontier` to analyze componentwise vector order among
  processed and unprocessed feature classes.
- Generated `width3-rank2221-vector-frontier.json`.
- Added `VECTOR_FRONTIER_NOTES.md`: the unprocessed vector classes have a
  single minimal frontier U6 with adjacent `(2,2,1)` and skip `(0,0,0)`, so the
  naive processed-class dominance story is false; the next target is a
...
- Continued P11 on `R013`.
- Extracted and derived the eighth-ranked `13/32` matrix bucket.
- Added named replay artifacts and depth-2 recurrence traces for both
  rank-normal forms.
- Added `EIGHTH_MATRIX_BUCKET_NOTES.md`, with exact count replay and a hand
...
- Continued P11 on `R013`.
- Extracted and derived the fifth-ranked singleton `2/5` matrix bucket.
- Added named replay artifacts and a depth-2 recurrence trace.
- Added `FIFTH_MATRIX_BUCKET_NOTES.md`, with exact count replay and a hand
  enumeration for `(c,d)`.
- Verdict remains `PARTIAL-PROGRESS`; no `CLAIMS.md` row.
...
## 2026-05-21T03:15:00+09:00

- Continued P11 on `R013`.
- Extracted and derived the fourth-ranked `2/5` matrix bucket.
- Added named replay artifacts and depth-2 recurrence traces for all four
  rank-normal forms.
- Added `FOURTH_MATRIX_BUCKET_NOTES.md`, with exact count table and a hand
  count for the equality form.
- Verdict remains `PARTIAL-PROGRESS`; no `CLAIMS.md` row.
...
- Continued P11 on `R013`.
- Extracted and derived the third-ranked `2/5` matrix bucket.
- Added named replay artifacts and depth-2 recurrence traces for all six
  rank-normal forms.
- Added `THIRD_MATRIX_BUCKET_NOTES.md`, with exact count table and a hand
  count for the equality form.
- Verdict remains `PARTIAL-PROGRESS`; no `CLAIMS.md` row.
...
- Continued P11 on `R013`.
- Added `named-case-recurrence` to expose minimal-element recurrence traces.
- Generated `case-a-recurrence.json`, `case-b-recurrence.json`, and
  `case-c-recurrence.json`.
- Added `RECURRENCE_LEDGER.md`, summarizing the branch recurrences for the
  three normal-form cases.
- Verdict remains `PARTIAL-PROGRESS`; no `CLAIMS.md` row.
...
- Continued P11 on `R013`.
- Added `cover-matrix-forms` to derive normal forms directly from rank shape
  and cover matrix.
- Generated `width3-rank2221-cover-matrix-forms-n7.json`.
- Added `COVER_MATRIX_DERIVATION.md`, recording that the common matrix under
  width 3 and height 4 has exactly three rank-normal forms.
- Verdict remains `PARTIAL-PROGRESS`; no `CLAIMS.md` row.
