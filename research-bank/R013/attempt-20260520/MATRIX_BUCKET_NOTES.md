...
The full bucket-member extraction is recorded in
`width3-rank2221-extremal-matrix-bucket-n7.json`, generated by:

```powershell
python tools\poset_balance.py bucket-members `
  --max-n 7 `
  --width 3 `
  --height 4 `
  --rank-shape 2,2,2,1 `
  --signature matrix `
  --bucket "layers=2,2,2,1|covers=8|mins=2|maxs=2|cover_matrix=[[0,2,2,0],[0,0,2,1],[0,0,0,1],[0,0,0,0]]" `
