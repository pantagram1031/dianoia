...
- New artifacts:
  ...
  `normal-form-cases/case-seventh-bucket-*`, and
  `SEVENTH_MATRIX_BUCKET_NOTES.md`.
- Finite signal: the seventh bucket has two rank-normal forms. Exact replay
  gives lower probabilities `19/41` and `13/32`; the weaker form is
...
- New artifacts:
  ...
  `normal-form-cases/case-fifth-bucket*`, and
  `FIFTH_MATRIX_BUCKET_NOTES.md`.
- Finite signal: the fifth bucket is singleton; exact replay counts give
  `(18,12)` over `30` extensions for `(c,d)`, lower probability `2/5`, with a
  hand enumeration in the note.
- Claim discipline: restricted subcase only; no `CLAIMS.md` row and no P12
  trigger.
...
- Continued P11 on `R013` by classifying the fourth-ranked matrix bucket in
  the restricted width-3, height-4, rank-shape `2,2,2,1` class.
- New artifacts:
  `width3-rank2221-fourth-matrix-bucket-n7.json`,
  `width3-rank2221-fourth-cover-matrix-forms-n7.json`,
  `normal-form-cases/case-fourth-bucket-*`, and
  `FOURTH_MATRIX_BUCKET_NOTES.md`.
- Finite signal: the fourth bucket has four rank-normal forms; exact replay
...
- New artifacts:
  `research-bank/R013/attempt-20260520/normal-form-cases/` and
  `research-bank/R013/attempt-20260520/NORMAL_FORM_COUNT_LEDGER.md`.
- Finite signal: each normal-form orientation count can now be replayed from
  named cover relations, not arbitrary search labels.
- Claim discipline: no new result, no `CLAIMS.md` row, no P12 trigger.
...
- Finite signal: the `14/39` extremal matrix bucket has exactly three members
  in the restricted `2,2,2,1` class, with best lower probabilities `14/39`,
  `14/33`, and `5/11`.
- Claim discipline: no new result, no `CLAIMS.md` row, no P12 trigger.

## 2026-05-20T23:30:00+09:00

- Extracted the three-profile matrix bucket for the R013 `14/39` extremal.
- New artifact:
  `research-bank/R013/attempt-20260520/MATRIX_BUCKET_NOTES.md`.
- Finite signal: within that bucket the three best lower probabilities are
  `14/39`, `14/33`, and `5/11`, so equality appears isolated to one profile.
- Claim discipline: no new result, no `CLAIMS.md` row, no P12 trigger.
