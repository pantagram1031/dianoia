# Benchmark Difference Evidence Verification - 2026-05-21

Purpose: require B6+ `VALUE_ADDED` comparisons to carry structured evidence for
three differences rather than only a prose assertion that dianoia helped.
...
- For B6+ `VALUE_ADDED` rows, `COMPARISON.md` must contain at least three
  numbered `## Three Differences` items.
- Each of the first three differences must include both `artifact:` and
  `quote:` evidence.
